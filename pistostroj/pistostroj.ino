...
void strihni() {
      ...
      cekejtisevestrihu(MINIMALNI_MOTOR);
...
/*        if (digitalRead(PIST_1_DOLE) == LOW) { //prehozeno
          if (KOMENTATOR) { Serial.print("Y"); }
...
void loop() {
     ...
     if ( digitalRead(LASER) == LOW ) { //prehozeno
        Serial.println("LASER ma matros");
      } else {
        Serial.println("LASER nema matros");
      }
 ...
 * 
      ...
      while (!(digitalRead(PIST_2_NAHORE) == HIGH)) {//prehozeno
          delay ( 1 );
      }
